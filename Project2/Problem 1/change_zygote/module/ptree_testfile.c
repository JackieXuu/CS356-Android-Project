...
void addNode(struct task_struct* cur, struct prinfo* buf, int id, bool flag)
{
	...
	buf[id].priority = cur->prio;
	buf[id].rt_priority = cur->rt_priority;
	buf[id].sched_method = cur->policy;
	if (hasChildren(cur))
	{
...
int dfs(struct prinfo* buf, int *nr)
{
	...
	while (cur != NULL)
	{
		if (strcmp(cur->comm,"main")==0) flag = true;
		if (strcmp(cur->comm,"gatekeeperd")==0) flag = false;
		...
		if (id < *nr) addNode(cur, buf, id++, flag);
		cur = getNext(cur);
		cnt++;
	}
	*nr = id;
